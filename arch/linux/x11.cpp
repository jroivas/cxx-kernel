...
bool X11::configure(ModeConfig *mode)
{
	...
	m_win = XCreateSimpleWindow(m_disp, DefaultRootWindow(m_disp),
		0, 0, mode->width, mode->height,
		0, white, black
		);
	//m_pix = XCreatePixmap(m_disp, DefaultRootWindow(m_disp), mode->width, mode->height, mode->depth);
	m_pix = XCreatePixmap(m_disp, m_win, mode->width, mode->height, mode->depth<32?mode->depth:24);
...
void X11::blit()
{
	XImage *img = XGetImage(m_disp, m_pix,
		0, 0, m_current->width, m_current->height,
		AllPlanes, ZPixmap);
	
	Mem::copy(img->data, m_buffer, img->bytes_per_line*img->height);

	XPutImage(m_disp, m_pix, m_gc, img, 
		0, 0, 0, 0, 
		m_current->width, m_current->height);
	XCopyArea(m_disp,m_pix,m_win,m_gc,0,0,m_current->width,m_current->height,0,0);
	XFree(img);
	XFlush(m_disp);
}
