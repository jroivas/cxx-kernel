    ...
    if (m_backbuffer != nullptr) {
        ...
        if (m_backbuffer[0] != 0xfe
            ...
            if (m_buffer == nullptr) {
                ...
                if (m_buffer[0] != 0xef || m_buffer[m_size / 2] != 0xde || m_buffer[m_size - 1] != 0xec) {
                    m_double_buffer = false;
                    MM::instance()->free(m_buffer);
                    m_buffer = nullptr;
                } else {
...
void FB::putPixel(int x, int y, unsigned char r, unsigned char g, unsigned char b, unsigned char a)
{
    ...
    if (x >= m_current->width) x = m_current->width - 1;
    if (y >= m_current->height) y = m_current->height - 1;
